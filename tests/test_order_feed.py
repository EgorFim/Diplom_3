...
class TestOrderFeed:
    ...
    def test_order_info_after_click(self,user_2,driver):
    ...
    def test_order_in_order_list(self,user_2,driver):
        login_page = LoginPage(driver)
        main_page = MainPage(driver)
    ...
    def test_counter_orders_of_all_time_change(self,user_2,driver):
        login_page = LoginPage(driver)
        main_page = MainPage(driver)
        order_feed_page = OrderFeedPage(driver)
        main_page.button_lenta_zakazov_click()
    ...
    @allure.description('Создаем заказ и проверяем что количество заказов после стало больше')
    def test_counter_today_orders_change(self,driver,user_2):
        login_page = LoginPage(driver)
        main_page = MainPage(driver)
        order_feed_page = OrderFeedPage(driver)
        main_page.button_lenta_zakazov_click()
